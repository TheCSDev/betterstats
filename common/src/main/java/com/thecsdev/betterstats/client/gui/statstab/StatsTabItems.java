...
/**
 * {@link StatsTab} that displays "Items" statistics.
 */
public sealed class StatsTabItems extends BetterStatsTab<ItemStats> permits StatsTabFood
{
	// ================================================== ==================================================
	//                                      StatsTabItems IMPLEMENTATION
	// ================================================== ==================================================
	public static final StatsTabItems INSTANCE = new StatsTabItems();
	// ==================================================
	protected StatsTabItems() {}
	// ==================================================
	 ...
	 */
	protected @Virtual void postProcessWidget(@NotNull StatsTab.StatsInitContext context, @NotNull TItemStatsWidget widget) {
		//obtain stats instance
		final var stats = widget.statsProperty().get();
		assert stats != null;
		//noinspection unchecked - context menu
		widget.contextMenuProperty().set((Function<TElement, TContextMenu>)(Object) CTX_MENU, StatsTabItems.class);
	}
	...
	public static final class SortBy implements TDropdownWidget.Entry
	{
		...
		{
			//initialize the list
			final var ist  = StatsProvider.getItemStatTypes();
	...
	public static enum GroupBy implements TDropdownWidget.Entry
	{
		...
		MOD(BSSLang.gui_statstab_filter_groupBy_mod(), stats -> {
		...
		TAB(BSSLang.gui_statstab_filter_groupBy_createiveModeTab(), stats -> {
		...
		// ==================================================
		/**
		 * Initializes GUI for the {@link StatsTabItems.GroupBy} filter.
		 * @param context The {@link StatsTab.FiltersInitContext}.
		 * @throws NullPointerException If the argument is {@code null}.
		 */
		public static final void initFilter(@NotNull StatsTab.FiltersInitContext context) throws NullPointerException
		{
			...
			final var dropdown = new TDropdownWidget<StatsTabItems.GroupBy>();
			dropdown.setBounds(nextY.x + 25, nextY.y, nextY.width - 25, nextY.height);
			Collections.addAll(dropdown.getEntries(), StatsTabItems.GroupBy.values());
			dropdown.tooltipProperty().set(__ -> TTooltip.of(BSSLang.gui_statstab_filter_groupBy()), GroupBy.class);
			panel.add(dropdown);

			//set initial value and apply filters on value update
			dropdown.selectedEntryProperty().set(
					context.getFilters().getProperty(StatsTabItems.GroupBy.class, FID, TAB),
					StatsTabItems.GroupBy.class);
			dropdown.selectedEntryProperty().addChangeListener((p, o, n) -> {
				context.getFilters().setProperty(StatsTabItems.GroupBy.class, FID, n);
				context.applyFilters();
			});
		}
		// ==================================================
