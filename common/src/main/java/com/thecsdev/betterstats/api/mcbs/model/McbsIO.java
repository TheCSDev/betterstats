 ...
 */
public final @ApiStatus.Experimental class McbsIO
{
	// ==================================================
	private McbsIO() {}
	// ==================================================
			...
			@NotNull McbsFile mcbsFile, @NotNull JsonObject json) throws NullPointerException
	{
		...
		for(final var statTypeEntry : mcbsFile.getStats().getIntValues().entrySet())
		{
			...
			for(final var statEntry : statTypeEntry.getValue().entrySet()) {
				if(statEntry.getValue() != 0) //do not waste storage with zeros
					json_stats_type.addProperty(statEntry.getKey().toString(), statEntry.getValue());
			}
		}
